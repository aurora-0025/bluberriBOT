...
module.exports = {
    ...
    function checkWin(p1,p2){
        ...
        .setColor('config.accentColor')
        .setFooter(`© ${message.guild.name}`);
        if(p1.choice==p2.choice) return message.channel.send({embeds: [drawEmbed]});
        if(p1.choice=="rock"&&p2.choice=="scissors") return message.channel.send({embeds: [p1winEmbed]});
        if(p1.choice=="paper"&&p2.choice=="rock") return message.channel.send({embeds: [p1winEmbed]});
        if(p1.choice=="scissors"&&p2.choice=="paper") return message.channel.send({embeds: [p1winEmbed]});
        if(p2.choice=="rock"&&p2.choice=="scissors") return message.channel.send({embeds: [p2winEmbed]});
        if(p2.choice=="paper"&&p2.choice=="rock") return message.channel.send({embeds: [p2winEmbed]});
        if(p2.choice=="scissors"&&p2.choice=="paper") return message.channel.send({embeds: [p2winEmbed]});
...
const filter = (interaction) => {
 ...
 interaction.reply({content:`${interaction.user.username} You already chose an option`,ephermal:true,}).then((msg)=>setTimeout(() => 
 msg.delete(), 3000));
return false;
    }
...
collector.on('collect', (ButtonInteraction)=>{
    ...
    .setColor('config.accentColor')
    .setFooter(`© ${message.guild.name}`);
message.channel.send({embeds:[waitEmbed]}).then((msg)=>setTimeout(() => 
msg.delete(), 3000)) 
    }
})
...
if(collected.size==2){
    ...
    choiceEmbed= new MessageEmbed()
        .setDescription(`${p1Choice.user} chose **${p1Choice.choice}**\n${p2Choice.user} chose **${p2Choice.choice}**`)
        .setColor('config.accentColor')
        .setFooter(`© ${message.guild.name}`);
...
if(collected.size==1){
    ...
    userChoice = {
        ...
        choice: array[0].value.customId}
    choiceEmbed= new MessageEmbed()
        .setDescription(`${userChoice.user} chose **${userChoice.choice}**\n${botChoice.user} chose **${botChoice.choice}**`)
        .setColor('config.accentColor')
        .setFooter(`© ${message.guild.name}`);
    message.channel.send({embeds:[choiceEmbed]})
