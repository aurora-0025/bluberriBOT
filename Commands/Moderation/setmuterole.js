...
module.exports = {
    name: "setmuterole",
    description: "To set the muterole\n ```usage: ;setmuterole <muteroleID>\n[aliases:smr]```",
    ...
    if(!args[0]){
        ...
        .setTitle('⚠️Please enter a role ID:\n'+'```'+';setmuterole <muteroleID>'+'```')
        .setColor('RED')
        .setFooter(`© ${message.guild.name}`);

      message.channel.send({embeds: [invalidFormatEmbed]}).then((msg)=>setTimeout(() => 
      msg.delete(), 10000)) 
    }
    else{
    var muterole=message.guild.roles.cache.find(r=>r.id===args[0]);

    if(!muterole){
        invalidIDEmbed= new MessageEmbed()
        .setTitle('⚠️Please enter a valid Role ID:\n'+'```'+';setmuterole <muteroleID>'+'```')
        .setColor('RED')
        .setFooter(`© ${message.guild.name} |this message will be deleted in 5 seconds`);

      message.channel.send({embeds: [invalidIDEmbed]}) .then((msg)=>setTimeout(() => 
      msg.delete(), 10000))
    }
    else{
        ...
        message.channel.send({embeds: [setroleEmbed]}).then((msg)=>setTimeout(() => 
        msg.delete(), 10000))
