     ...
     return message.channel.send({embeds: [errEmbed]}).then((msg)=>setTimeout(() => 
      msg.delete(), 5000)) 
        ...
        if(!message.guild.me.permissions.has("BAN_MEMBERS")) {
            errEmbed= new MessageEmbed()
            .setTitle(`⚠️**${message.author.username}**, I do not have perms to unban someone`)
            .setColor('RED')
        ...
        if (!userID) {
            ...
            .setTitle(`⚠️Please enter a userID to unban someone\`${config.prefix}unban <userID>\`\n use \`${config.prefix}banlist\` to find the id of banned members`)
            .setColor('RED')
            .setFooter(`© ${message.guild.name} |this message will be deleted in 10 seconds`);
          ...
          message.guild.bans.fetch().then(bans=> {
            ...
            if(!bUser) {
                invalidMentionEmbed= new MessageEmbed()
                .setTitle(`⚠️Please enter a valid userID to unban someone \`${config.prefix}unban <userID>\`\n use \`${config.prefix}banlist\` to find the id of banned members`)
                .setColor('RED')
                .setFooter(`© ${message.guild.name} |this message will be deleted in 10 seconds`);
