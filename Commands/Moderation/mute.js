...
module.exports = {
    ...
    run: async (client, message, args) => {
       ...
       if(!muteRoleID){
        ...
        .setTitle('⚠️Please set a valid mute Role ID:'+'```'+';setmuterole <muteroleID>'+'```')
        .setColor('RED')
        .setFooter(`© ${message.guild.name}`);

        return message.channel.send({embeds: [roleNotSetEmbed]}).then((msg)=>setTimeout(() => 
        msg.delete(), 10000))        
    }
        ...
        if (target) {
            ...
            let muteRole = message.guild.roles.cache.find(role => role.id === muteRoleID);
            if(!muteRole){
                roleNotSetEmbed= new MessageEmbed()
                .setTitle('⚠️Please set a valid mute Role ID:'+'```'+';setmuterole <muteroleID>'+'```')
                .setColor('RED')
    ...
    if (!reason) {
      reason = "None"
            ...
            .setTitle(`${memberTarget.user.tag} has been muted for ${ms(ms(args[1]))}`)
            .setColor(config.accentColor)
            .setFooter(`© ${message.guild.name}`)
            ...
            .setTitle('⚠️Cant find that member!'+"```"+"\n;mute <mention> <time in s,m,h,d,y>"+"```")
            .setColor('RED')
