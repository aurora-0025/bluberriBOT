...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (!args[0]) {
      ...
      .setTitle('⚠️Please mention the user you want to ban and specify a ban reason.\n'+'```'+';ban <mention> <reason>'+'```')
      .setColor('RED')
      .setFooter(`© ${message.guild.name} |this message will be deleted in 10 seconds`);

    return message.channel.send({embeds: [noMentionEmbed]}).then((msg)=>setTimeout(() => 
    msg.delete(), 10000)) 
		}
    ...
    if(member.permissions.has("ADMINISTRATOR")||member.permissions.has("BAN_MEMBERS")){
      errEmbed= new MessageEmbed()
      .setTitle('⚠️You cant ban this user |this message will be deleted in 5 seconds')
      .setColor('RED')
      ...
      .setTitle('⚠️Please use a proper mention if you want to ban someone.')
      .setColor('RED')
      .setFooter(`© ${message.guild.name} |this message will be deleted in 5 seconds`);

    return message.channel.send({embeds: [invalidMentionEmbed]}).then((msg)=>setTimeout(() => 
    msg.delete(), 5000)) 
		}
    ...
    return message.channel.send({embeds: [failEmbed]}).then((msg)=>setTimeout(() => 
    msg.delete(), 5000)) 
		}
